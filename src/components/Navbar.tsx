  ...
  return (
    <nav
      className={cn(
        ...
        scrolled || menuOpen ? 'bg-pg-dark/95 backdrop-blur-md shadow-lg' : 'bg-transparent'
      )}
    >
...
const MobileNavLink: React.FC<NavLinkProps> = ({ to, children, active }) => {
  return (
    <Link
      to={to}
      className={cn(
        "text-white font-medium text-lg py-1 hover:text-pg-purple transition-colors duration-300",
        active && "text-pg-purple"
      )}
    >
      {children}
